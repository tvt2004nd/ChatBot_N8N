...
export default function FilePanel() {
  ...
  const getFileIconInfo = (fileName: string) => {
    ...
    if (["pdf"].includes(ext)) return { icon: <BsFileEarmarkPdf />, colorClass: style.iconRed };
    if (["xlsx", "xls", "csv"].includes(ext)) return { icon: <BsFileEarmarkExcel />, colorClass: style.iconGreen };
    if (["docx", "doc", "txt"].includes(ext)) return { icon: <BsFileEarmarkWord />, colorClass: style.iconBlue };
                ...
                return (
                  <li key={file.file_id} className={style.fileItem}>
                    ...
                    <div className={style.fileDetails}>
                      <span className={style.fileName} title={file.file_name}>{file.file_name}</span>
                      <span className={style.fileTime}>
                        {new Date(file.modified_time).toLocaleDateString('vi-VN', {
                    ...
                    <div className={style.fileActions}>
                      ...
                      >
                        {deletingId === file.file_id ? (
                          <span className={style.uploadSpinner} style={{ width: 14, height: 14, borderWidth: '1.5px' }}></span>
                        ) : (
                          <BsTrash />
                        )}
                      </button>
